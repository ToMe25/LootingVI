...
public class EnchantmentDamageOW extends Enchantment
{
    ...
    public EnchantmentDamageOW(Enchantment.Rarity rarityIn, int damageTypeIn, EquipmentSlotType... slots)
    {
    ...
    {
        return this.damageType == 0 ? 1.0F + (float)Math.max(0, level - 1) * 0.5F : (this.damageType == 1 && creatureType == CreatureAttribute.UNDEAD ? (float)level * 2.5F : (this.damageType == 2 && creatureType == CreatureAttribute.ARTHROPOD ? (float)level * 2.5F : 0.0F));
    }
    ...
    {
        return !(ench instanceof EnchantmentDamageOW);
    }
